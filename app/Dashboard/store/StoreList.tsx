...
const StoreList = () => {
  ...
  const {
    ...
    queryKey: ["get-admin-list-store", tokenStore],
    enabled: !!tokenStore,
  });
  ...
  const handleConfirmStore = useCallback(
    async (id: string) => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/auth/verify-store`,
          {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const handleDeclineStore = useCallback(
    async (id: string, reason: string) => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/auth/declined-store`,
          {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${token}`,
                        ...
                        <Dialog>
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent className="w-24">
                              ...
                              {row.original.is_verified === "WAITING" ? (
                                <>
                                  <DropdownMenuItem
                            ...
                            <DialogContent className="sm:max-w-md">
                              ...
                              <DialogFooter className="sm:justify-start">
                                ...
                                <Button
                                  variant="lime"
                                  onClick={() => {
                                    handleConfirmStore(row.original.user_id);
                                  }}
                                >
                                  Confirm
                                </Button>
                          ...
                          {dialogReason === true && (
                            <DialogContent>
                              <DialogHeader>
                                ...
                                <DialogDescription>
                                  Please enter the reason why you decline this
                                  store.
                                </DialogDescription>
                              </DialogHeader>
